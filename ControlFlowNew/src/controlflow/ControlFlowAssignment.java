...
public class ControlFlowAssignment {
	public static void main (String[]args) {
		...
		if((status=='A'&&income<=8350.00||status=='C'&&income<=8350.00)||status=='B'&&income<=16700.00||status=='D'&&income<11950.00) {

			taxRate=.10;
			System.out.println("your income tax is "+ "$"+df.format(taxRate*income));
		}else if((status=='A'&&income<=33950.00||status=='C'&&income<=33950.00)||(status=='B'&&income>=16701.00&&income<=67900.00)||status=='D'&&income>=11951&&income<=45500.00) {
			taxRate=.15;
			System.out.println("your income tax is "+ "$"+df.format(taxRate*income));
